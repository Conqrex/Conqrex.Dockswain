...
# mobile-export INCLUDE_SECRETS INDEX_OR_all SERVERS_JSON DOCKER_CMD NGINX_DIR SSH_TIMEOUT
# Emits a compact dockswain://import QR payload for the MAUI app. Secrets are opt-in
# because QR codes are visible plaintext: password auth pulls from the same keyring
# lookup path used for connecting; key auth embeds the private-key file contents only
# when a readable key path is configured.
if [ "$SUB" = "mobile-export" ]; then
    INC="${1:-0}"; WHICH="${2:-all}"; SJ="${3:-[]}"
    DC="${4:-docker}"; ND="${5:-/etc/nginx}"; TO="${6:-5}"
    command -v jq >/dev/null 2>&1 || emit_err "missing_jq"
    [[ "$TO" =~ ^[0-9]+$ ]] || TO=5

    selected=$(printf '%s' "$SJ" | jq -c --arg which "$WHICH" '
        if type != "array" then []
        elif $which == "all" then .
        else [ .[($which|tonumber)] ] | map(select(. != null))
        end' 2>/dev/null) || emit_err "bad_json"
    ...
    while IFS= read -r s; do
        [ -n "$s" ] || continue
        label=$(printf '%s' "$s" | jq -r '.label // ""')
        user=$(printf '%s' "$s" | jq -r '.user // ""')
        host=$(printf '%s' "$s" | jq -r '.host // ""')
        port=$(printf '%s' "$s" | jq -r '.port // 22')
        auth=$(printf '%s' "$s" | jq -r '.auth // "key"')
        key=$(printf '%s' "$s" | jq -r '.key // ""')
        use_sudo=$(printf '%s' "$s" | jq -r 'if .useSudo then "true" else "false" end')
        remmina=$(printf '%s' "$s" | jq -r '.remmina // ""')
        [ -n "$host" ] || continue
        [[ "$port" =~ ^[0-9]+$ ]] || port=22
        target="${host}"; [ -n "$user" ] && target="${user}@${host}"
        mobile_auth="Password"; [ "$auth" = "key" ] && mobile_auth="PrivateKey"

        password=""; private_key=""
        if [ "$INC" = "1" ]; then
            if [ "$auth" = "password" ]; then
                if command -v secret-tool >/dev/null 2>&1; then
                    password=$(secret-tool lookup service cnq-dockswain host "${target}:${port}" 2>/dev/null)
                    if [ -z "$password" ]; then
                        if [ -n "$remmina" ] && [ -f "$remmina" ]; then
                            password=$(remmina_secret "$remmina")
                        else
                            rf=$(remmina_file_for "$target" "$port")
                            [ -n "$rf" ] && password=$(remmina_secret "$rf")
                        fi
                    fi
                fi
                if [ -n "$password" ]; then secret_n=$((secret_n + 1))
                else warns+=("$(jq -Rn --arg v "$target:$port password not found" '$v')"); fi
            elif [ -n "$key" ]; then
                kp="$key"
                case "$kp" in "~/"*) kp="$HOME/${kp#~/}" ;; esac
                if [ -r "$kp" ]; then
                    private_key=$(cat -- "$kp" 2>/dev/null)
                    [ -n "$private_key" ] && secret_n=$((secret_n + 1))
                else
                    warns+=("$(jq -Rn --arg v "$target:$port key file not readable" '$v')")
                fi
            else
                warns+=("$(jq -Rn --arg v "$target:$port uses ssh-agent; mobile needs a pasted private key" '$v')")
            fi
        fi

        out+=("$(jq -nc \
            --arg label "$label" --arg user "$user" --arg host "$host" \
            --argjson port "$port" --arg auth "$mobile_auth" --argjson sudo "$use_sudo" \
            --arg docker "$DC" --arg nginx "$ND" --argjson timeout "$TO" \
            --arg password "$password" --arg privateKey "$private_key" \
            '{l:$label,u:$user,h:$host,p:$port,a:(if $auth == "PrivateKey" then "k" else "p" end),
              su:$sudo,dc:$docker,nd:$nginx,to:$timeout}
             + (if $password != "" then {pw:$password} else {} end)
             + (if $privateKey != "" then {pk:$privateKey} else {} end)')")
        n=$((n + 1))
    done < <(printf '%s' "$selected" | jq -c '.[]')
    ...
    payload=$(jq -nc --argjson servers "$servers" \
        '{t:"dsw",v:1,s:$servers}')
    b64=$(printf '%s' "$payload" | base64 | tr -d '\n' | tr '+/' '-_' | tr -d '=')
    qr="DSWAIN1:${b64}"
    if [ "${#qr}" -gt 1800 ]; then
        warns+=("$(jq -Rn --arg v "QR is very dense; if scanning fails, export servers one by one or without private keys" '$v')")
    fi
    if [ "${#warns[@]}" -gt 0 ]; then
        warnings=$(printf '%s\n' "${warns[@]}" | jq -sc '.')
    else
    ...
    jq -nc --arg payload "$qr" --argjson count "$n" --argjson secrets "$secret_n" \
        --argjson warnings "$warnings" \
        '{ok:true,payload:$payload,count:$count,secrets:$secrets,warnings:$warnings}'
    exit 0
fi
